#!/usr/bin/env python3
"""
Baut COMPANY-OS.bin -- das BIOS fuer Firmenrechner.
...
def bauen(still=False):
    ...
    ziel = os.path.join(HIER, "COMPANY-OS.bin")
    daten, _ = asm_file(quelle, ziel, os.path.join(HIER, "COMPANY-OS.sym"))
    if len(daten) > ROM_SIZE:
        raise SystemExit(
            f"COMPANY-OS passt nicht ins ROM: {len(daten)} Bytes, Platz sind "
            f"{ROM_SIZE}.")
    daten, summe = bios_kopf_stempeln(ziel)
    if not still:
        print(f"  COMPANY-OS {len(daten):6d} Bytes  "
