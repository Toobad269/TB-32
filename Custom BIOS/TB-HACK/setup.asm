...
; TB-HACK: die sechs Zeilen des Reiters Hack
.equ REG_HKMON,    0xE4            ; Knopf: Hex-Monitor
.equ REG_HKPORT,   0xE5            ; Knopf: Portkonsole
.equ REG_HKCMOS,   0xE6            ; Knopf: CMOS-Editor
...
setup_load_defaults:
    ...
    movi r11, 0
    call cmos_write
    ; TB-HACK: F5 ist hier auch die Notbremse. Wer den Startsektor auf etwas
    ; Unbootbares gestellt oder sich einen Patch eingebaut hat, der den Kernel
    ; zerlegt, kommt so mit einer Taste zurueck auf einen startenden Rechner.
    ; Die Patchadressen selbst bleiben stehen -- sie werden nur nicht mehr
    ; angewandt, und das ist die Aenderung, die man rueckgaengig machen will.
    movi r10, CM_HKBSEC0
    movi r11, 0
    call cmos_write
    movi r10, CM_HKBSEC1
...
; --- TB-HACK: die Werkzeuge -----------------------------------------------
;  Vier davon nehmen den ganzen Bildschirm ein. Danach muss der Setup-Rahmen
;  neu gezeichnet werden, sonst steht die Liste auf dem Bild des Werkzeugs.
.hkmon:
    call hk_monitor
    call setup_frame
    jmp .done
...
.hkcmos:
    ...
    jmp .done
.hksec:
    call hk_sektor                    ; fragt nach dem Sektor und laedt ihn ...
    call hk_monitor                   ; ... und zeigt ihn gleich an
    call setup_frame
    jmp .done
.hkbsec:
    call hk_bootsektor
...
; --- TB-HACK: der Startsektor, vierstellig hex wie ueberall im Reiter Hack
.hkbsec:
...
; Neun Zeilen -- mehr passen nicht in den Rahmen. Der Kasten ist 14 hoch und
; faengt bei y=3 an, die Zeilen bei SET_Y=6: Zeile 9 laege auf dem unteren
; Rand.
tab_hack:
    .dw s_e_hkmon,  REG_HKMON,    0, 0
    .dw s_e_hkport, REG_HKPORT,   0, 0
    .dw s_e_hkcmos, REG_HKCMOS,   0, 0
    .dw s_e_hksec,  REG_HKSEC,    0, 0
    .dw s_e_hkbsec, REG_HKBSEC,   0, 0
    .dw s_e_hknos,  CM_HKNOSIG,   2, opts_onoff
...
s_e_hkmon:   .db "Memory Monitor", 0
s_e_hkport:  .db "Port Console", 0
s_e_hkcmos:  .db "CMOS Editor", 0
s_e_hksec:   .db "Load Disk Sector", 0
s_e_hkbsec:  .db "Boot Sector", 0
s_e_hknos:   .db "Ignore Boot Signature", 0
