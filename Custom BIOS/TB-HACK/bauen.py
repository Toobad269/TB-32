#!/usr/bin/env python3
"""
Baut TB-HACK.bin -- das BIOS-Abbild mit den Bastlerwerkzeugen.

    python3 "Custom BIOS/TB-HACK/bauen.py"
...
def bauen(still=False):
    ...
    ziel = os.path.join(HIER, "TB-HACK.bin")
    daten, _ = asm_file(quelle, ziel, os.path.join(HIER, "TB-HACK.sym"))
    if len(daten) > ROM_SIZE:
        raise SystemExit(
            f"TB-HACK passt nicht ins ROM: {len(daten)} Bytes, Platz sind "
            f"{ROM_SIZE}.")
    daten, summe = bios_kopf_stempeln(ziel)
    if not still:
        print(f"  TB-HACK {len(daten):6d} Bytes  "
