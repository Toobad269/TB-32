...
/* Ein Fenster ist entstanden oder verschwunden -- der Schreibtisch malt
   sich in seiner eigenen Schleife neu. Steht hier, weil dialog.c vor gui.c
   eingebunden wird und beide es brauchen. */
int fw_wunsch = 0;
int dlg_neu = 0;
int dlg_status = 0;             /* 0 offen, 1 gewaehlt, 2 abgebrochen */
int dlg_rueck = 0 - 1;          /* wessen Fenster danach wieder obenauf */
...
void dlg_oeffne(int modus, char* endung, char* vorschlag) {
    dlg_status = 0;
    dlg_rueck = win_top;
    /* Der Aufruf kommt aus dem Prozess des PROGRAMMS. Der Schreibtisch muss
       sich selbst neu malen, sonst steht das Dialogfenster zwar in der
       Liste, ist aber nicht zu sehen -- derselbe Fall wie bei fw_neu. */
    fw_wunsch = 1;
    dlg_modus = modus;
    dlg_sel = 0;
    dlg_top = 0;
...
void dlg_schliessen() {
    ...
    /* Die Tastatur gehoert wieder dem Programm, das gefragt hat. Welches das
       war, merkt sich dlg_rueck -- ohne das zeigt win_top auf ein
       geschlossenes Fenster und jeder Tastendruck versickert. */
    if (dlg_rueck >= 0 && win_type[dlg_rueck] != 0) win_top = dlg_rueck;
}

/* Der Rueckweg.

   Frueher rief der Dialog beim Auftraggeber eine Funktion auf -- das ging,
   solange Editor, Paint und Word IM KERNEL standen. Jetzt sind sie
   eigenstaendige Programme, und ein Programm kann man nicht einfach
   anspringen. Also legt der Dialog sein Ergebnis hin, und wer gefragt hat,
   holt es sich ab (Systemaufrufe 90 und 91). Genau so arbeitet der
   gemeinsame Dateidialog bei den grossen Systemen auch.

   dlg_status: 0 = offen, 1 = etwas ausgewaehlt, 2 = abgebrochen. */
