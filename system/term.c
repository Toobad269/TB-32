/* ==========================================================================
   ...
   The command line should also run on the desktop. For that it needs two
   things:

     1. Its own screen buffer. Everything that would otherwise go to the
        text screen ends up in TERM_BUF -- the UI then paints it as a window.
     2. Its own keyboard. The UI reads the keys and passes them on to the
        terminal whenever its window is currently in front.

   The shell itself runs as its own process. While it waits for input it
   yields the CPU -- so the UI stays responsive, and the clock and system
...
/* Scrollback: the lines that scroll off the top move into a ring buffer.
   Without it they would simply be gone -- the text-mode command line has
   had this for a long time, the window did not yet. */
#define TERM_SB     0x00124000
#define TERM_SBMAX  200

int term_aktiv = 0;              /* 1 = output goes to the window */
int term_x = 0;
int term_y = 0;
int term_attr = 7;
int term_dirty = 1;
int term_lauf = 0;               /* is the shell process already running? */
int term_pid = 0 - 1;

/* --- Capturing output ------------------------------------------------------
   While the editor has something compiled, the compiler runs as its own
   process and writes its messages to wherever output currently goes -- in
   graphics mode that means the invisible text screen. But those error
   messages are exactly what you want to see. So the output can
   additionally be captured into a buffer that the compile window then
   displays. */
...
int cap_aktiv = 0;
int cap_zeile = 0;               /* next line to be written */
int cap_spalte = 0;
int cap_voll = 0;                /* how many lines are in it */
...
void cap_putc(int c) {
    int a;
    if (cap_zeile >= CAP_ZEILEN) return;         /* full: the rest is dropped */
    if (c == 13) return;
...
int term_sb_count = 0;           /* how many lines are saved */
int term_sb_head = 0;            /* next write slot in the ring */
int term_view = 0;               /* 0 = live, otherwise that many lines back */
...
/* Save the top line into the ring buffer before it gets overwritten */
void term_sb_push() {
    int i; int z;
...
/* Address of the i-th visible line. view says how many lines have been
   scrolled back; the calculation runs over an imagined combined stream of
   ring buffer + current screen, so the transition is seamless. -1 means:
...
/* Output a number (the shell otherwise uses the BIOS service) */
...
void term_push_key(int k) {
    ...
    if (neu == term_khead) return;               /* buffer full */
...
/* Waits for a key and yields the CPU while doing so, so the UI keeps
   running. */
int term_getkey() {
    int k;
